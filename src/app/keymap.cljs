   ...
   (->>
    (conj
      ...
      }
     {
      ...
      :Cmd-Tab (fn [editor] (alternate-active {:active @(rf/subscribe [:active-file])
                                              :editor editor :open-files open-files}))
      :Cmd-S (fn [editor] (fs/save file (.getValue (.-doc editor))))
      :Shift-Ctrl-S (fn [editor] (fs/save-ide-settings))
      :Cmd-Q (fn [editor] (next-active {:active @(rf/subscribe [:active-file])
                                       :editor editor :open-files open-files}))
